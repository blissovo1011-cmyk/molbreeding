...
export async function publish(req: Request, res: Response, next: NextFunction) {
  try {
    const reagent = reagentService.publishReagent(req.params.id, req.body);
    res.json({ success: true, data: reagent });
  } catch (err) { next(err); }
}
...
export async function offline(req: Request, res: Response, next: NextFunction) {
  try {
    const reagent = reagentService.offlineReagent(req.params.id, req.body);
    res.json({ success: true, data: reagent });
  } catch (err) { next(err); }
}
...
export async function remove(req: Request, res: Response, next: NextFunction) {
  try {
    reagentService.deleteReagent(req.params.id);
