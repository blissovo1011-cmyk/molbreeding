...
export function findByReagentAndSystem(reagentId: string, system: SubSystem): ReagentSyncConfig | null {
  const db = getDb();
  const row = db.prepare('SELECT * FROM reagent_sync_configs WHERE reagentId = ? AND system = ?').get(reagentId, system);
  return (row as ReagentSyncConfig) || null;
}
...
export function upsert(
  ...
  db.prepare(
    `INSERT INTO reagent_sync_configs (id, reagentId, system, alertValue, warehouse, kingdeeCode, localName, status) VALUES (?,?,?,?,?,?,?,?)`
